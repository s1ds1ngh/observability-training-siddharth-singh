...
public class InventoryController {
    ...
    public InventoryController(MeterRegistry meterRegistry, Tracer tracer) {
    ...
    public ResponseEntity<Map<String, Object>> getItem(@PathVariable String itemId) {
        ...
        try {
            ...
            // Custom nested span with attribute to trace business logic
            Span customSpan = tracer.nextSpan().name("validate-inventory").start();
            try (Tracer.SpanInScope spanInScope = tracer.withSpan(customSpan)) {
                customSpan.tag("item.id", itemId);
                // Simulate some validation work
                if (random.nextInt(100) < 10) {
                    throw new IllegalStateException("Validation failed for itemId=" + itemId);
                }
            } finally {
                customSpan.end();
            }
